...
class ConcurrentRotatingFileHandler(BaseRotatingHandler):
    ...
    def __init__(self, filename, mode='a', maxBytes=0, backupCount=0,
                 encoding=None, debug=True, delay=0, zipCompress=True, zipBackupCount=10):
        """
        ...
        self.zipCompress=zipCompress
        self.zipBackupCount=zipBackupCount
        self._open_lockfile()
    ...
    def doCompress(self):
        if not self.zipCompress:
            return
        oldestfile = "%s.%d" % (self.baseFilename, self.backupCount)
        if not os.path.exists(oldestfile):
            return
        compressid = str(uuid.uuid1())
        for i in range(self.backupCount - 1, -1, -1):
            sfn = "%s.%d" % (self.baseFilename, i + 1)
            dfn = "%s.%d.%s" % (self.baseFilename, i + 1, compressid)
            os.rename(sfn, dfn)
        for i in range(self.zipBackupCount - 1, 0, -1):
            sfn = "%s.%d.zip" % (self.baseFilename, i)
            dfn = "%s.%d.zip" % (self.baseFilename, i + 1)
            if os.path.exists(sfn):
                #print "%s -> %s" % (sfn, dfn)
                if os.path.exists(dfn):
                    os.remove(dfn)
                os.rename(sfn, dfn)
        dfn = self.baseFilename + ".1.zip"
        if os.path.exists(dfn):
            os.remove(dfn)
        zfile = zipfile.ZipFile(dfn, "w", zipfile.ZIP_DEFLATED)
        for i in range(self.backupCount):
            sfn = "%s.%d.%s" % (self.baseFilename, i + 1, compressid)
            #print "Compress %s -> %s" % (sfn, dfn)
            if os.path.exists(sfn):
                zfile.write(sfn)
                os.remove(sfn)
        zfile.close()
    ...
    def doRollover(self):
        ...
        if self.backupCount <= 0:
            ...
            return
        try:
            # Check if backupCount file reached, if reached, compress backupCount files to zip,
            # then delete then.
            try:
                self.doCompress()
            except:

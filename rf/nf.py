...
# 注册nf包 √
def nf_package(package_url, package_id, filename, description): 
    print(package_url)
    # prepare data
...
# 查询所有的nf包 √
def get_nf_packages(package_url):
    package_url = str(package_url)
...
# 查询nf包是否存在 √
def nf_package_exist(package_url, package_id):
    nf_packages = get_nf_packages(package_url)
...
# 根据id查询nf包信息 √
def get_nf_package_by_package_id(package_url, package_id):
...
# 删除nf包 √
def delete_nf_package(package_url, package_id):
    nf_package_disable(package_url, package_id)
...
# disable package √
def nf_package_disable(package_url, package_id):
...
# 查询nf是否存在 √
def nf_exist_return_id(nfs_url, nfinstname):
    nfs = send_get_nfs(nfs_url)
...
# 发送实例化请求 √
def send_create_nf(create_url, data):
